<script lang="ts">
	...
	// The query returns newest-first; render oldest-first so the newest message
	// sits at the bottom, matching conventional chat layout.
	let orderedMessages = $derived([...displayedMessages].reverse());

	const client = useConvexClient();
	const messageCharacterLimit = 140;

	// Reflect the live options below in the displayed snippet.
	let searchArg = $derived(`{ searchWords: [${searchWords.map((w) => `'${w}'`).join(', ')}] }`);
	let codeSnippet = $derived(`const messages = usePaginatedQuery(
  api.messages.paginatedList,
  ${skipQuery ? "'skip'" : searchArg},
  { initialNumItems: 3, keepPreviousData: ${useStale} }
);
...
client.mutation(api.messages.send, { author, body }, {
  optimisticUpdate: (store) => {
	...
	// Show the live demo by default; the code is one click away.
	let tab: 'demo' | 'code' = $state('demo');

	// Bottom-anchored scrolling: keep the view pinned to the newest message
	...
	function handleScroll() {
		if (!scrollEl) return;
		stickToBottom = scrollEl.scrollHeight - scrollEl.scrollTop - scrollEl.clientHeight < 48;
	}

	$effect(() => {
		// Re-run when the message list changes, scrollEl mounts, or we switch
		// back to the demo tab (the scroller is hidden while viewing code).
		void orderedMessages.length;
		void tab;
		if (!scrollEl || tab !== 'demo' || !stickToBottom) return;
		void tick().then(() => {
	...
	function onSubmit(e: SubmitEvent) {
		...
		toSend = '';
		// Always follow your own message down to the bottom.
		stickToBottom = true;
...
<div class="flex w-full flex-col">
	<div class="flex items-center gap-1 border-b border-convex-ink/10 bg-convex-sand/40 px-3 py-2">
		<button
			type="button"
			onclick={() => (tab = 'demo')}
			class="cursor-pointer rounded-full px-3 py-1 text-xs font-medium transition-colors {tab ===
			'demo'
				? 'bg-white text-convex-ink shadow-sm'
				: 'text-convex-ink/50 hover:text-convex-ink'}"
		>
			Demo
		</button>
		<button
	...
	<div class:hidden={tab !== 'code'}>
		<CodePanel code={codeSnippet} />
	</div>

	<div class="flex flex-col" class:hidden={tab !== 'demo'}>
		...
		<div
			...
			class="flex h-96 flex-col overflow-y-auto px-5 py-4"
		>
				...
				<div class="mt-auto flex flex-col gap-4">
					{#if messages.status !== 'Exhausted'}
						<div class="flex justify-center">
							...
							>
								{messages.status === 'LoadingMore' ? 'Loading…' : 'Load older'}
							</button>
						</div>
					{/if}
					<ul class="flex flex-col gap-4">
						{#each orderedMessages as message (message._id)}
